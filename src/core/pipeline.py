...
class StockAnalysisPipeline:
    ...
    def analyze_stock(
        ...
        try:
            ...
            self._emit_progress(18, f"{code}: 시세와 기본 데이터를 불러오는 중")
            ...
            self._emit_progress(32, f"{stock_name}: 기본 정보와 추세 데이터를 모으는 중")
            ...
            self._emit_progress(46, f"{stock_name}: 뉴스와 시장 심리를 검색하는 중")
            ...
            self._emit_progress(58, f"{stock_name}: 분석에 필요한 가격 데이터를 정리하는 중")
            ...
            def _on_llm_stream(chars_received: int) -> None:
                ...
                self._emit_progress(
                    dynamic_progress,
                    f"{stock_name}: LLM이 분석 결과를 작성하는 중 ({chars_received}자 수신)",
                )

            self._emit_progress(64, f"{stock_name}: LLM에 분석 리포트를 요청하는 중")
            ...
            if result:
                self._emit_progress(94, f"{stock_name}: 분석 결과를 검토하고 정리하는 중")
